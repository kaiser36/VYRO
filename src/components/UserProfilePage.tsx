...
export const UserProfilePage: React.FC<UserProfilePageProps> = ({
  ...
  const { currentUser, logout, updateProfile, toggleFavorite } = useUser();
  ...
  return (
    <div className="min-h-screen bg-neutral-50 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        ...
        {activeTab === 'rewards' && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-neutral-900 via-neutral-900 to-black text-white p-8 rounded-3xl relative overflow-hidden border border-neutral-800">
              ...
              <div className="relative z-10 max-w-xl">
                <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  VYRO Kinetic Rewards
                </span>
                <h2 className="font-serif text-3xl font-semibold mt-3 mb-2">
                  O Teu Clube de Atleta
                </h2>
                <p className="text-xs text-neutral-400 mb-6">
                  Cada 1€ gasto em meias VYRO equivale a 10 pontos. Acumula pontos em cada treino e desbloqueia descontos e meias de edições especiais.
                </p>

                <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
                  <div>
                    <span className="text-[11px] text-neutral-400 block">Saldo Atual</span>
                    <span className="font-serif text-3xl font-bold text-cyan-400">
                      {currentUser.points || 0}
                    </span>
                    <span className="text-xs text-neutral-400 ml-1">pontos</span>
                  </div>
                  <div className="h-10 w-px bg-white/10 mx-2" />
                  <div>
                    <span className="text-[11px] text-neutral-400 block">Estatuto</span>
                    <span className="font-semibold text-white text-sm">{currentUser.tier}</span>
                    <span className="text-[10px] text-emerald-400 block">Acesso Prioritário Ativo</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white p-5 rounded-2xl border border-neutral-200/80 flex flex-col justify-between">
                <div>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800">
                    250 Pontos
                  </span>
                  <h3 className="font-serif text-lg font-semibold text-black mt-2">Envio Grátis Imediato</h3>
                  <p className="text-xs text-neutral-500 mt-1">Portes grátis em qualquer encomenda, sem valor mínimo.</p>
                </div>
                <button
                  disabled={(currentUser.points || 0) < 250}
                  className="mt-4 w-full py-2 bg-neutral-100 hover:bg-neutral-200 disabled:opacity-40 text-black text-xs font-semibold rounded-xl transition-colors cursor-pointer"
                >
                  {(currentUser.points || 0) >= 250 ? 'Resgatar Cupão' : 'Faltam pontos'}
                </button>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-neutral-200/80 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-serif text-lg font-semibold text-black mt-2">Vale de 5€ Desconto</h3>
                  <p className="text-xs text-neutral-500 mt-1">Desconto direto no carrinho em qualquer modelo de meias.</p>
                </div>
                <button
                  disabled={(currentUser.points || 0) < 500}
                  className="mt-4 w-full py-2 bg-neutral-100 hover:bg-neutral-200 disabled:opacity-40 text-black text-xs font-semibold rounded-xl transition-colors cursor-pointer"
                >
                  {(currentUser.points || 0) >= 500 ? 'Resgatar Cupão' : 'Faltam pontos'}
                </button>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-neutral-200/80 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-serif text-lg font-semibold text-black mt-2">Par de Meias Grátis</h3>
                  <p className="text-xs text-neutral-500 mt-1">Um par de meias VYRO da tua escolha incluído na encomenda.</p>
                </div>
                <button
                  disabled={(currentUser.points || 0) < 1000}
                  className="mt-4 w-full py-2 bg-neutral-100 hover:bg-neutral-200 disabled:opacity-40 text-black text-xs font-semibold rounded-xl transition-colors cursor-pointer"
                >
                  {(currentUser.points || 0) >= 1000 ? 'Resgatar Recompensa' : 'Faltam pontos'}
                </button>
              </div>
            </div>
          </div>
